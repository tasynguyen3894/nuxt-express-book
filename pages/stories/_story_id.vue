            ...
            <div class="flex w-1/5 border-gray-400 border-r py-2 pl-2 mr-5">
                <ul class="block w-full">
                    <li :key="chap.id" v-for="chap in story.chaps">
                        <nuxt-link @click="changeChapId(chap.id)" v-bind:class="[chap.id == chap_id ? 'text-teal-500' : '']" class="py-1 hover:text-teal-500 block" :to="{name: 'stories-story_id-chaps-chap_id', params: {story_id: story_id, chap_id: chap.id} }">
                            {{ chap.name }}
                        </nuxt-link>
                    </li>
                </ul>
